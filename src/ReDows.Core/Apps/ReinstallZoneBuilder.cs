...
/// too-shallow path (fewer than three segments, such as a bare volume or a single-folder
...
public static class ReinstallZoneBuilder
{
    ...
    public static IReadOnlyList<ReinstallZone> Build(AppInventoryReport inventory)
    {
        ...
        foreach (var app in inventory.Entries)
        {
            ...
            // root, so the zone's own leaf must be vetted here, keeping default-to-review).
