...
/// ReDows to InDows loop: the apps catalog (configuration.dsc.yaml), the settings
...
public static class ProfileCommand
{
    ...
    private static int RunCore(string[] options)
    {
        ...
        // Settings half: load the catalog, read the live machine (read-only).
        ...
        Console.WriteLine(
            ...
            $"{profile.Manual.Count + profile.NotApplied.Sum(m => m.Settings.Count) + profile.PrivateOnly.Count} setting(s) need a manual touch. See README.md.");
        return 0;
    }
}
